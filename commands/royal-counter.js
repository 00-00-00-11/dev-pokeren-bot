  ...
  if (args[0] === 'edit' && isNaN(args[1])) {
    ...
    RoyalCount.findOneAndUpdate(
      ...
      (err, count) => {
        ...
        return message.channel.send(`Onze discord server heeft tot nu toe al ${count.count} Royal flushes gehad!`);
      }
    );
  } else if (args[0] === 'add') {
    // Add 1 to count
      ...
      (err, count) => {
        ...
        if (!count) {
          ...
          return message.channel.send(
            `Gefeliciteerd <@${message.author.id}> met je Royal flush! Dit is royal flush nummer ${count.count} van onze ${message.guild.name} server!`
          );
        }
      }
    );
